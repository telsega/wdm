...
FILE *WDMLogStream(FILE * debugfile)
{
...
void WDMUseSysLog(const char *ident, int facility)
{
	if (!ident)
		ident = "wdm";
...
static int WDMLevelToSyslog(int level)
{
	switch (level) {
...
int WDMStringToFacility(const char *facility)
{
...
static void WDMLogMessageRaw(int level, char *fmt, va_list args)
{
...
void WDMLogMessage(int level, char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
